...
public class HomeController extends HttpServlet{
	...
	private void sellerHome(HttpServletRequest req, HttpServletResponse resp, int sellerID) throws ServletException, IOException {
		...
		int orderCanceled=listOrders.stream().filter(OrderModel-> OrderModel.getStatus()==5).collect(Collectors.toList()).size();
		int orderUnconfirm=listOrder.size();
		int orderUnpre=listOrderUnpre.size();
		int orderComplete=listOrders.stream().filter(OrderModel->OrderModel.getStatus()==4 && OrderModel.getPayment().getStatus()==1).collect(Collectors.toList()).size();
